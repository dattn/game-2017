        ...
        this.$game
            ...
            .then(() => {
                ...
                this.$socket.on("start", ({ id, position, colorIndex }) => {
                    ...
                    this.$map
                        .loadChunksByPosition(position.x, position.y)
                        .then(() => {
                            this.$player.addTrait(
                                new UpdateCameraTrait(this.$game.camera)
                            );
                            this.$player.addTrait(
                                new UpdateBackgroundTrait(this.$background)
                            );
                            this.$player.addTrait(
                                new ChunkLoaderTrait(this.$map)
                            );
                            this.$player.addTrait(
                                new PositionDialogTrait(this.$game)
                            );
                ...
                this.$socket.on("update", data => {
                    ...
                    Object.keys(data.player).forEach(key => {
                        ...
                        if (
                            ...
                            this.$player.body.importState(
                                data.player[key].data,
                                0.25
                            );
                            return;
                        }
                        if (!this.$remotePlayers.hasOwnProperty(key)) {
                            this.$remotePlayers[key] = this.$game.createEntity(
                                "Player",
                                {
                                    colorIndex: data.player[key].colorIndex
                                }
                            );
                            this.$remotePlayers[key].addTrait(
                                new AttachTextTrait(
                                    this.$game,
                                    data.player[key].name
                                )
                            );
                        }
                        this.$remotePlayers[key].body.importState(
                            data.player[key].data
                        );
                        this.$remotePlayers[key].isRunning =
                            data.player[key].isRunning;
                        this.$remotePlayers[key].isJumping =
                            data.player[key].isJumping;
                    });

                    data.chunks.forEach(chunk => {
                        if (this.$map.hasChunk(chunk.x, chunk.y)) {
                            this.$map._handleChunkData(
                                chunk.x,
                                chunk.y,
                                chunk.data
                            );
